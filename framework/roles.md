...
**A dispatched agent inherits the parent's git credentials** - the SSH agent socket, the `gh` token, and any keychain-stored path all carry over, so a worktree only isolates the tree, not the remote. A write-capable agent can push to any branch it can reach with those credentials. There is no dispatch-time control for this: the outbound path is held by instruction plus checking the tree and the remote after the agent finishes, not by anything that blocks the push before it happens.
