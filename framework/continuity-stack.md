...
| deviations register - `portfolio/<front>/[<project>/]deviations.md` | **Every drift from what was approved** - Problem / Why / Resolution / Anti-patterns rejected, one row plus one body section | by `/log-deviation`, BEFORE the fix is attempted | when a similar drift recurs · at `/retro` |
| epic folder - `tasks/<EPIC-ID>-<slug>/` (stage docs + `30-fragments/*`) | **An epic's plan and its execution state** - the stage documents carry the locked plan, each fragment brief carries its own state (`pending / in-progress / squashed / invalidated`) | by `/epic-flow` at each stage; brief states at each child boundary | resuming an epic - a cold session reads the folder, never a memory |
| lesson-candidate buffer - the ticket's evidence log | **Unvetted candidates in transit** - a handoff so an 11:00 lesson survives to a 19:00 debrief; writes to NO lesson store | by `/mission-flow` Phase 8 as it reports | by `/debrief` before `/learn-from-session` runs |
...
- **That check runs in BOTH directions.** The rule above keeps the reference layer from duplicating itself. The other direction keeps the lesson stores from duplicating the reference layer: `/learn-from-session`'s dedup rule reads the touched front's hub and spoke alongside the two lesson indexes, because a fact `/mission-flow` Phase 8 wrote to a spoke is invisible from `MEMORY.md` and the learning log. The general form: a skill writing to one layer checks every layer that could already own the fact, not only its own.
- **The buffer is not a store.** The lesson-candidate line on a ticket holds candidates only until the debrief's critique gate accepts or rejects them; it never becomes a third lesson store, and a candidate the gate rejects simply dies there.
- **The register is evidence, not memoir.** A deviation is logged BEFORE its fix is attempted; logged afterwards it is a story, and the anti-patterns-rejected field - the teaching half - is exactly what gets lost.
