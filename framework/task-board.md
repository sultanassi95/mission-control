...
## Epic-sized work - the folder above a ticket

Work whose correct shape is several tickets is not filed as one large task. It
runs `/epic-flow`, which plans it and then executes each piece as a full
mission-flow child. Two things follow for the board.

**The epic is a folder beside the tickets**, in the same `tasks/` directory
(front-level when the epic crosses projects):

```
tasks/<EPIC-ID>-<slug>/
  ticket.md            the epic-level ticket (external tracker: pointer + DoD)
  00-face.md           the outcome, non-goals, and the epic terminal assertion
  10-current-state.md  what exists, as evidence
  20-architecture.md   boundaries, contracts, and "## Declared outputs"
  30-fragments/        one brief per fragment - the state machine
  40-coverage.md       GENERATED by tools/epic-coverage.sh, never hand-edited
  50-sequence.md       lane order, walking skeleton first
  60-tree-lock.md      the founder's lock = the execution grant
```

**The fragment brief is the truth, not the child ticket.** Each brief carries
its own frontmatter state (`pending | in-progress | squashed | invalidated`),
its `depends_on`, and its `outputs` - the deliverables that fragment OWNS,
never merely files it touches. The child ticket that mission-flow creates
points AT the brief (`> Tracked in: the epic brief - <path>`) and carries only
the Definition-of-Done checklist and its evidence log, so criteria live once.

**EPIC is a size, not just a shape.** `/triage` sizes items S / M / L / EPIC;
an EPIC item is handed `/epic-flow` rather than filed as a task, because a
single task file cannot hold work whose plan needs a coverage gate. Filing it
as one L task is how a plan's remainder gets lost.

**Coverage is mechanical.** `tools/epic-coverage.sh <epic-folder>` diffs the
union of fragment `outputs:` against the architecture's declared list and exits
1 on an unclaimed remainder or an over-claim (two fragments owning one output).
A remainder gets a fragment before execution starts, never as a follow-up.
